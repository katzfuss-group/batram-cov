...
from .typing import AnomalizedData, AppData
...
def create_and_merge_problems(
    ...
    location_subset: Sequence[int] | None = None,
) -> rp.HGPIPProblem:
    ...
    if sample_subset is None:
        sample_subset = np.arange(data.samples.shape[1])
    ...
    problems = []
    assert location_subset is not None
    for i in location_subset:
        x = data.x
        nn_idx = data.nearest_neighbors[i, :max_num_nn]

        try:
            data.samples[sample_idx][:, nn_idx]
        except Exception as e:
            raise type(e)(
                f"data.samples[sample_idx][:, nn_idx] failed. {sample_idx=}, {nn_idx=}"
                f"{data.samples.shape}, {nn_idx}"
            )
...
def build_data_module(data: AnomalizedData, config: dict) -> rp.TMDataModule:
...
def fit_model(
    ...
    config: dict,
    run: Run,
    score_data: AnomalizedData | None = None,
...
def predict_and_plot(
    ...
    if x_shape == 1:
        x, samples = predictions.cmip_app(
            rng_key, model, dataset, sample_fixed_noise, num_samples=5
        )

        fig = plotting.plot_app_run(x, samples, desc=desc)
    else:
        raise NotImplementedError(
            f"Predictions and plotting for {x_shape} are not implemented."
        )

    return fig


def anomalize_data(data: AppData | None) -> AnomalizedData | None:
    if data is None:
        return None

    mean = data.samples.mean(axis=-2, keepdims=True)
    sd = data.samples.std(axis=-2, keepdims=True)
    anomalies = (data.samples - mean) / sd
    return AnomalizedData(
        locs=data.locs,
        nearest_neighbors=data.nearest_neighbors,
        li=data.li,
        maxmin_permutation=data.maxmin_permutation,
        x=data.x,
        samples=anomalies,
        mean=mean,
        sd=sd,
...
def load_app_data(
    config: dict[str, str], path: str | os.PathLike
) -> tuple[
    str,
    AnomalizedData,
    AnomalizedData,
    ...
    hashed_filename = sha512(str(config).encode()).hexdigest()[:8]

    # defining logic to apply in the next mapping function
    def from_pickle(filename: str | None) -> AnomalizedData | None:
        if filename is None:
            return None
    ...
    train, validation, test = map(lambda f: from_pickle(f), files)

    assert isinstance(train, AnomalizedData), (
        f"Expected AnomalizedData, got {type(train)=}"
    )
    assert isinstance(validation, AnomalizedData), (
        f"Expected AnomalizedData, got {type(validation)=}"
    )
    assert isinstance(test, AnomalizedData), (
        f"Expected AnomalizedData, got {type(test)=}"
    )

    return hashed_filename, train, validation, test
...
def do_one_run(
    ...
    with wandb.init(
        ...
        hashname, *datasets = load_app_data(data_config, parsed_args.data_path)

        train_data, validation_data, test_data = datasets
        if not isinstance(train_data, AnomalizedData):
            logging.error(
                f"Bad training data input. Got {type(train_data)} instead of AnomalizedData."
            )
            run.finish(1)

        n = train_data.samples.shape[1]
        if sample_subset_size > 0 and sample_subset_size < n:
            logging.info(f"Using a subset of size {sample_subset_size} for training.")
            sample_subset = np.random.choice(n, sample_subset_size, replace=False)
            train_data._replace(samples=train_data.samples[:, sample_subset, :])

        logging.info(
            f"Create and merge {train_data.samples.shape[-1]} regression problems"
        )
        logging.info(f"training with {train_data.samples.shape[1]} samples")

        model = create_and_merge_problems(train_data, model_config)

        logging.info("Fitting model.")
        model, train_loss, validation_loss, success = fit_model(
        ...
        # if test_data:
        #     fig = predict_and_plot(
        #         rng_key=jax.random.key(seeds[2]),
        #         model=model,
        #         dataset=test_data,
        #         sample_fixed_noise=sample_fixed_noise,
        #         desc=description,
        #     )
        #     log_media_with_run(fig, run)
        #     plt.close(fig)

    hash_model_config = sha512(str(config["model"]).encode()).hexdigest()[:8]
    hashname = f"{hashname}_{hash_model_config}"
    save_experiment(
        model=model,
        data_dict=dict(
            train_data=train_data, valid_data=validation_data, test_data=test_data
        ),
        config=config["model"],
...
def save_experiment(
    ...
    data_dict: dict[str, AnomalizedData],
    config: dict[str, Any],
    ...
    if test_data is None:
        raise ValueError(
            "Expected test_data to be provided in `data_dict` but it is missing."
        )

    preds = predictions.just_predict(model, test_data)
    x, fixed_noise_samples, random_noise_samples = jax.tree.map(jax.device_get, preds)

    score_data_module = build_data_module(test_data, config)
    log_probs = log_prob_with_data(model, score_data_module)
    fs, gs = get_fs_and_gs(model, score_data_module)
    log_probs, fs, gs = jax.tree.map(jax.device_get, (log_probs, fs, gs))

    name = os.path.join(data_path, hashed_name + "_inference.pkl")
    logging.info(f"saving inference to {name}")
    # INFO:root:saving inference to {hash}_inference.pkl
    ...
    name = os.path.join(data_path, hashed_name + "_data.pkl")
    logging.info(f"saving data to {name}")
    # INFO:root:saving data to {hash}_data.pkl

    with open(name, "wb") as f:
        pickle.dump(data_dict, f)

    name = os.path.join(data_path, hashed_name + "_config.pkl")
    logging.info(f"saving config to {name}")
    # INFO:root:saving config to {hash}_config.pkl
...
def main(args: list[str] | None = None):
    parser = argparse.ArgumentParser(
        prog="app",
        description="Runs app using a config.yaml file",
    )
    parser.add_argument("--config", type=str, help="path to config file to load")
    parser.add_argument(
        "--data-path",
        type=str,
        default="/mnt/data/research/transport-maps/tmcov/app-data",
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="size of the subset of locations to use for training (-1: None)",
    )
    parser.add_argument(
        "--sample-fixed-noise",
        action="store_true",
    ...
    if not os.path.exists(parsed_args.data_path):
        raise RuntimeError(f"Data path not found: {parsed_args.data_path}")
    ...
    for i, config in enumerate(configs):
        logging.info(f"Running experiment {i} out of {len(configs)}.")
        do_one_run(
            parsed_args,
            config,
            i,
            parsed_args.sample_fixed_noise,
            parsed_args.subset_size,
        )
        # try:
        #     do_one_run(
        #         parsed_args,
        ...
        #     )
        # except Exception as e:
        #     logging.error(f"Error {e} in experiment {i}. Config file:\n")
